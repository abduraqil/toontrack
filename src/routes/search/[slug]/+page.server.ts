...
async function resolveQuery(
    ...
    switch (slug) {
        case 'cartoons':
            ...
            q_y1 =
                year && year > 1908
                    ? gte(tbl.airStart, new Date(year, 0, 1))
                    : undefined
            q_y2 =
                yearMax && yearMax < CURRENTYEAR
                    ? lte(tbl.airStart, new Date(yearMax + 1, 0, 0))
                    : undefined // last day of the year

            /* status */
            q_s =
                status === 0 || status === null || status === undefined
                    ? undefined
                    : eq(tbl.status, status)
            break
        case 'staff':
    ...
    if (tbl === cartoons || tbl === companies || tbl === staff) {
        ...
        formatsOrOccupations = await db
            .select({ id: ttbl.id, name: ttbl.name })
            .from(ttbl)
            .orderBy(ttbl.name)
        ...
        if (tbl === cartoons || tbl === staff) {
            ...
            q_l =
                language === null || language === undefined
                    ? undefined
                    : eq(languages.id, language)
            // get all languages
            allLangauges = await db
    ...
    if (tbl === cartoons) {
        ...
        query
            .leftJoin(
                jtCartoonsCartoonTypes,
                eq(jtCartoonsCartoonTypes.fkCartoonId, tbl.id)
            )
            .leftJoin(
                cartoonTypes,
                eq(jtCartoonsCartoonTypes.fkCartoonTypeId, cartoonTypes.id)
            )
            /* languages for cartoons */
            .leftJoin(
                jtCartoonsLanguages,
                eq(jtCartoonsLanguages.fkCartoonId, tbl.id)
            )
            .leftJoin(
                languages,
                eq(jtCartoonsLanguages.fkLanguageId, languages.id)
            )
    }
    if (tbl === companies) {
        /* types for companies */
        query
            .leftJoin(
                jtCompaniesCompanyTags,
                eq(jtCompaniesCompanyTags.fkCompanyId, tbl.id)
            )
            .leftJoin(
                companyTags,
                eq(jtCompaniesCompanyTags.fkCompanyTagId, companyTags.id)
            )
    }
    if (tbl === staff) {
        ...
        query
            .leftJoin(
                jtOccupationsStaff,
                eq(jtOccupationsStaff.fkStaffId, tbl.id)
            )
            .leftJoin(
                occupations,
                eq(jtOccupationsStaff.fkOccupationId, occupations.id)
            )
            /* languages for voice actors */
            .leftJoin(jtLanguagesStaff, eq(jtLanguagesStaff.fkStaffId, tbl.id))
            .leftJoin(
                languages,
                eq(jtLanguagesStaff.fkLanguageId, languages.id)
            )
    }
    query
        .where(and(q_n, q_y1, q_y2, q_s, q_l, q_t))
