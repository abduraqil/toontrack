...
import { eq, ilike, count, asc, desc, and, lte, gte, sql, getTableColumns } from 'drizzle-orm';
import {
    users,
...
async function resolveQuery(
    ...
    let query,
        q,
        tbl, // main search table
        ljtbl, // languages junction table
        tjtbl,
        ttbl, // type/occupation junction table & origin table
        formatsOrOccupations,
        allLangauges = null,
        q_y1, q_y2, q_s, q_l, q_t, q_n // query parameters
        ;
    ...
    * NOTE: q_X must be undefined if that query parameter is unused, this is accepted by and() */
    ...
    let order = (!ascending) ? desc(tmp) : asc(tmp);
    ...
    const totalResults: number = query.length,
        totalPages: number = Math.ceil(totalResults / numResults);
...
function parseURL(url: URL) {
    ...
    let ascending = (tmp != "false" || tmp == null) ? true : false;

    tmp = parseInt(url.searchParams.get("year1")!);
    let year = isNaN(tmp) ? 0 : tmp;

    tmp = parseInt(url.searchParams.get("year2")!);
    let yearMax = isNaN(tmp) ? (new Date()).getFullYear() : tmp;

    tmp = parseInt(url.searchParams.get("type")!);
    let format = isNaN(tmp) ? undefined : tmp;

    tmp = parseInt(url.searchParams.get("status")!);
    let status = (isNaN(tmp) || !(tmp <= 6 && tmp >= 0)) ? 0 : tmp;

    tmp = parseInt(url.searchParams.get("language")!);
    let language = isNaN(tmp) ? undefined : tmp;
...
export const load: PageServerLoad = async ({ url, params }) => {
    ...
    let { url: u, name, numResults, currentPage, sort, ascending, year, yearMax, format, status, language } = parseURL(url)
    let ur = u.toString()

    try {
        let { query, totalResults, totalPages, formatsOrOccupations, allLangauges } = await resolveQuery(numResults, currentPage, slug, name, sort, ascending, year, yearMax, format, status, language)
