    ...
    <div class="prose max-w-none">
                    ...
                    <!-- <a href="/cartoons/{company.cartoon.id}"> -->
                        <div class="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 flex overflow-hidden h-24">
                            <div class="flex-1 p-4 flex flex-col justify-center">
                                    <a href="/companies/{company.id}" class="font-semibold text-lg text-gray-900 mb-1 line-clamp-1">{company.name}</a>
                                <div class="flex flex-wrap gap-1">
                                        <span class="inline-block bg-purple-100 text-gray-800 text-xs px-2 py-1 rounded-full">
                                            {convertCompany(company.role)}
                                        </span>
                                </div>
                            </div>
                            <div class="w-24 h-24">
        ...
        {:else}
            <div class="bg-gray-50 rounded-lg p-8 text-center">
                ...
                <p class="text-gray-600 text-lg">No company information available.</p>
            </div>
        {/if}
    </div>
</div>
