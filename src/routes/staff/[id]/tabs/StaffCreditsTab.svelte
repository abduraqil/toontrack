<script lang="ts">
    ...
    */
    ...
    // function groupStaffByName(staffRole: any[]) {
    //     const grouped = new Map();
    //
    //     staffRole.forEach(staffRelation => {
    //         // Access the staff data through the relation
    //         const role = staffRelation.role;
    //         const id = staffRelation.fkCartoonId;
    //
    //         if (!role) return; // Skip if staff data is missing
    //
    //         const characterName = staffRelation.ca;
    //         if (grouped.has(name)) {
    //             grouped.get(name).roles.push(role);
    //         } else {
    //             grouped.set(name, { // include the staff properties you need
    //                 name: name,
    //                 birthday: staffMember.birthday || 'Unknown',
    //                 coverPic: staffMember.coverPic || null,
    //                 roles: [role],
    //                 id: id
    //             });
    //         }
    ...
    // $: groupedStaff = staffMember.jtCartoonsStaff && staffMember.jtCartoonsstaffMember.length > 0
    // ? groupStaffByName(staffMember.jtCartoonsStaff)
    // : [];
</script>
...
<!--                                 <h3 class="font-semibold text-lg text-gray-900 mb-1 line-clamp-1">{staffMember.name}</h3> -->
<!--                                 <div class="flex flex-wrap gap-1"> -->
<!--                                     {#each staffMember.roles as role} -->
